...
def add_data():
    ...
    for row in rows:
        cur.execute(f"INSERT INTO phone_book (name, phone_number) VALUES ('{row[0]}', '{row[1]}')")
...
def update_data():
    ...
    cur.execute(f"SELECT * FROM phone_book WHERE name = '{initial_name}' AND phone_number = '{initial_number}'")
    if cur.rowcount == 0:
        print("Contact does not exist")
        return

    mode = input("Update:\nname - update name\nnumber - update number\nEnter: ")
    if mode not in ["name", "phone_number"]:
        print("Error")
        return
    ...
    cur.execute(f"UPDATE phone_book SET {mode} = '{new_info}' WHERE name = '{initial_name}' AND phone_number = '{initial_number}'")
...
def query_data():
    ...
    query_dict = {
        ...
        2 : f"phone_number = '{query_value}'",
        3 : f"starts_with(name, '{query_value}') OR starts_with(phone_number, '{query_value}')",
        4 : f"name ILIKE '%{query_value}%' OR phone_number ILIKE '%{query_value}%'"
        }

    cur.execute(f"SELECT * FROM phone_book WHERE {query_dict[query_mode]}")
...
def delete_data():
    ...
    cur.execute(f"DELETE FROM phone_book WHERE {mode} = '{info}'")
